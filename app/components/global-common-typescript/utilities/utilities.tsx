...
export function coalesce(...args: Array<string | null | undefined>): string | null {
...
export function kvpListToObject<T>(kvpList: Array<[string, T]>): {[attribute: string]: T} {
...
export function deterministicShuffle<T>(array: Array<T>, seed: number): Array<T> {
...
export function getSingletonValue<T>(arr: Array<T>, error: string): T {
    if (arr === undefined || arr === null || arr.length == 0) {
        throw new Error(`Zero values received, when one and only one was expected: ${JSON.stringify(arr)}; ${error}`);
    }

    if (arr.length > 1) {
        throw new Error(`More than one value received, when one and only one was expected: ${JSON.stringify(arr)}; ${error}`);
    }

    return arr[0];
}

export function getSingletonValueOrNull<T>(arr: Array<T>, error: string): T | null {
    if (arr === undefined || arr === null || arr.length == 0) {
        return null;
    }
